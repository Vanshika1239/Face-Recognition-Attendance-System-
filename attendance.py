...
class Attendance:
    def __init__(self, root):
        ...
        self.var_atten_enrollment = StringVar()
        ...
        img1 = Image.open(r"C:\Users\admin\PycharmProjects\pythonProject1 face recognition.py\123.PNG")
        img1 = img1.resize((1535, 120), Image.ANTIALIAS)
        self.photoimg1 = ImageTk.PhotoImage(img1)

        f_lbl = Label(self.root, image=self.photoimg1)
        f_lbl.place(x=0, y=0, width=1535, height=120)
        ...
        title_lbl = Label(bg_img, text="ATTENDANCE MANAGE SECTION", font=("times new roman", 18, "bold"), bg="white",
                          fg="midnightblue")
        title_lbl.place(x=0, y=0, width=1535, height=32)

        main_frame = Frame(bg_img, bd=3, bg="white")
        main_frame.place(x=5, y=35, width=1535, height=525)

        # left frame
        Left_frame = LabelFrame(main_frame, bd=3, bg="white", relief=RIDGE, text="Students Attendance  Details",
                                font=("times new roman", 12, "bold"))
        Left_frame.place(x=2, y=1, width=660, height=518)

        img_left = Image.open(r"C:\Users\admin\PycharmProjects\pythonProject1 face recognition.py\att.PNG")
        ...
        # Roll
        enrollmentLabel = Label(left_inside_frame, text="Enrollment:", font=("times new roman", 12, "bold"),
                                bg="white")
        enrollmentLabel.grid(row=0, column=2, padx=2, pady=4, sticky=W)

        atten_enrollment = ttk.Entry(left_inside_frame, width=20, textvariable=self.var_atten_enrollment,
                                     font=("times new roman", 12, "bold"))
        atten_enrollment.grid(row=0, column=1, padx=8, pady=4, sticky=W)
        ...
        atten_dep = ttk.Entry(left_inside_frame, textvariable=self.var_atten_dep, width=20,
                              font=("times new roman", 12, "bold"))
        ...
        atten_time = ttk.Entry(left_inside_frame, textvariable=self.var_atten_time, width=20,
                               font=("times new roman", 12, "bold"))
        ...
        save_button = Button(btn_frame, text="Import csv", command=self.importCsv, width=17,
        ...
        update_button = Button(btn_frame, text="View Attendance Report ", command=self.viewReport, width=18,
                               font=("times new roman", 12, "bold"), bg="navy", fg="white")
        update_button.grid(row=0, column=1)

        delete_button = Button(btn_frame, text="Update", width=17, command=self.action,
                               font=("times new roman", 12, "bold"), bg="navy", fg="white")
        delete_button.grid(row=0, column=2)

        reset_button = Button(btn_frame, text="Reset", command=self.reset_data, width=17,
                              font=("times new roman", 12, "bold"), bg="navy", fg="white")
        ...
        "id", "name", "enrollment","department", "time", "date", "attendance"), xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.AttendanceReportTable.column("enrollment",width=120)
    ...
    def importCsv(self):
        ...
        with open(fln) as myfile:
            csvread = csv.reader(myfile, delimiter=",") ##, tha
            for i in csvread:
                mydata.append(i)
    ...
    def action(self):
        ...
        name=self.var_atten_enrollment.get()
        enrollment=self.var_atten_name.get()
        ...
        try:
            ...
            with open(fln,mode="a",newline="\n") as f:
                ...
                ["StudentID","Name","Enrollment","Department","Time","Date","Attendance"]))
...
# view attendance report csv
    ...
    def get_cursor(self, event=""):
        ...
        self.var_atten_enrollment.set(rows[2])
